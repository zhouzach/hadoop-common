...
object SparkMysqlConnector {
  ...
  def main(args: Array[String]): Unit = {
    //    checkConnectivity()
//    readFromMysql()
    readFromMysqlByPushdownUnionQuery()
  ...
  def readFromMysqlByPushdownUnionQuery() = {
    // Note: The parentheses are required.
    val query = """
      |(
      |select * from bill_202009 where id > 73000
      |union all
      |select * from bill_202010 where id > 44120
      |) bill
      |""".stripMargin
    spark.read.jdbc(url = jdbcUrl, table = query, properties = connectionProperties)
      .show()
  }
