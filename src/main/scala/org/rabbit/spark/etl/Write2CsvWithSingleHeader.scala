...
import org.rabbit.spark.sql.CaseWhen.sparkSession
...
object Write2CsvWithSingleHeader {
  ...
  def main(args: Array[String]): Unit = {
    ...
    headerDF.union(dataStrDF).write.mode(SaveMode.Overwrite)
      .option("header", "false").csv("/Users/Zach/org.rabbit.hadoop-common/output")

    //use org.rabbit.hadoop FileUtil to merge all partition csv files into a single file
    val fs = FileSystem.get(sparkSession.sparkContext.hadoopConfiguration)
    FileUtil.copyMerge(fs, new Path("/Users/Zach/org.rabbit.hadoop-common/output"),
      fs, new Path("/Users/Zach/org.rabbit.hadoop-common/target.csv"),
